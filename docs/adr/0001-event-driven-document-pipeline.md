...
2. **It can't scale the OCR path independently of the rest of the system.** Eligibility matching (`/api/match`) is fast, in-memory Rust -- true when this decision was made, and still true of the matching logic itself. It has since gained one network hop (a cached fetch from `catalog-service`, see the README's "Scheme data" section) when the scheme catalog moved into Postgres, but that hop is still nothing next to a vision-API call: cached, sub-5s-timeout, and matching never blocks on OCR either way. Document extraction is slow and I/O-bound. Bolting the two into the same process means the only way to handle more OCR load is to run more copies of the *whole* api-gateway, including the parts that didn't need scaling.
