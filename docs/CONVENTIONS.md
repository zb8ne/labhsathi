...
`labhsathi-core::schemes::SCHEMES` is a flat `&'static [Scheme]` array, iterated by `match_schemes`. Appending an entry is the entire change — no restructuring needed. Each `Scheme` needs an `id`, `name`, `authority`, `benefit`, a `documents` list, an `official_note`, and a `rule: fn(&UserProfile) -> Option<String>` closure returning the plain-language match reason.
